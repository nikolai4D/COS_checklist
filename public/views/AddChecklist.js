...
export default class AddChecklist {
  ...
  async getTemplate() {
    ...
    let allOmraden = (await State.allOmraden)
    ...
    let allFragetyper = (await State.allFragetyper)

    allFragetyper.sort((a, b) => a.title.localeCompare(b.title))

    let frageTyperStr = "";
    allFragetyper.forEach(fragetyp => {
      frageTyperStr += `
    ...
    console.log(frageTyperStr)




    // await Actions.GET_ALL_FRAGOR();
