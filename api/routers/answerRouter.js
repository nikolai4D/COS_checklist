    ...
    for (const answer of questions){
        for (const question of answer.questions){
            if (question.selectedAnswer){
                let matchingAnswer = questionsWithAnwers.find(obj => obj.question.parentId === question.id)
                if (matchingAnswer){
                    if (matchingAnswer.answer.parentId !== question.selectedAnswer){
                        await apiCallDelete(`instance/${matchingAnswer.answer.id}`);
                    }
                ...
                //         title: question.answers.find(answer => answer.parentId === question.selectedAnswer).title,
                //         checklist: id
                //     }
                    // await apiCallPost("answers", newAnswer);
            }
